    ...
    <div :class="['text-left plan-node', {'detailed': showDetails, 'never-executed': isNeverExecuted, 'parallel': workersCount}]">
      ...
      <div class="plan-node-body">
        ...
        <div v-if="shouldShowRowsRemoved">
          <span>
            Rows removed:
            <b><span :class="'p-0 px-1 alert ' + rowsRemovedClass">{{ rowsRemoved }}</span></b>
            out of {{ rowsRemoved + node[nodeProps.ACTUAL_ROWS] }}
            ({{ rowsRemovedPercent }}%</span>),
            {{ node[nodeProps.ACTUAL_ROWS] }} returned
          </span>
        </div>
...
export default class PlanNode extends Vue {
  ...
  private calculateRowsRemoved() {
    const rowsRemovedKey = _.find(_.keys(this.node), (key) => _.startsWith(key, 'Rows Removed'));
    if (rowsRemovedKey) {
